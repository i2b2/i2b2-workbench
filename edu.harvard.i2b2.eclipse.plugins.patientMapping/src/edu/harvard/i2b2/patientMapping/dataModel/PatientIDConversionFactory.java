...
public class PatientIDConversionFactory {
	...
	public String convertLine(String input, int inputIndex, int outputIndex) {
		...
		if(inputIndex == 1 && outputIndex == 1) {
			...
			output = convert55(input);
		}
		else if(inputIndex == 6) {
			output = convert66(input);
		}
	...
	private String convert55(String input) {
		String output = "";
		String[] cols = input.split(",");
		String id;
		if(cols.length < 10) {
			...
			id = encryptID(cols[0]);//, 1, 1);					
		}
		output = new String(id);//+","+cols[1]+","+id+"\n");
		for(int i=1;i<10;i++) {
			output+=","+cols[i];
		}
		output += "\n";
	...
	private String convert66(String input) {
		String output = "";
		String[] cols = input.split(",");
		String id;
		if(cols.length < 11) {
