...
import edu.harvard.i2b2.patientSet.ui.ConceptTreePanel;
import edu.harvard.i2b2.patientSet.ui.TopPanel;
...
public class TopPanelModel {
	...
	public String wirteQueryXML() {
		...
		for (int i = 0; i < panelCount; i++) {
			...
			ArrayList<QueryConceptTreeNodeModel> nodelist = panel.data()
					.getItems();
			if ((nodelist != null) && (nodelist.size() > 0)) {
				...
				for (int j = 0; j < nodelist.size(); j++) {
					QueryConceptTreeNodeModel node = nodelist.get(j);
		...
		try {
			...
			for (int i = 0; i < numOfPanels; i++) {
				...
				for (int j = 0; j < panelType.getItem().size(); j++) {
					...
					QueryConceptTreeNodeModel nodedata = new QueryConceptTreeNodeModel();
