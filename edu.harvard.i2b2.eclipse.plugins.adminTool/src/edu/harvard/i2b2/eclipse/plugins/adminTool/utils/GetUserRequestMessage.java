...
public class GetUserRequestMessage extends ProjectManagementRequestData {
	
	public static final String THIS_CLASS_NAME = GetUserRequestMessage.class.getName();
	...
	public GetUserRequestMessage(String uname, PasswordType pword, String domain) {
	...
	public BodyType getBodyType(String user) {
		edu.harvard.i2b2.crcxmljaxb.datavo.pm.ObjectFactory of = new edu.harvard.i2b2.crcxmljaxb.datavo.pm.ObjectFactory();
		
		BodyType bodyType = new BodyType();
		bodyType.getAny().add(of.createGetUser(user));//createSetPassword(str));//"i2b2"));
	 ...
	 * @return A String data type containing the Ont RequestMessage in XML format
	 */
	public String doBuildXML(String user){ 
		String requestString = null;
			try {
				MessageHeaderType messageHeader = getMessageHeader(); 
				...
				messageHeader.setSecurity(securityType);
				RequestHeaderType reqHeader  = getRequestHeader();
				BodyType bodyType = getBodyType(user) ;
				RequestMessageType reqMessageType = getRequestMessageType(messageHeader,
						reqHeader, bodyType);
				requestString = getXMLString(reqMessageType);
			} catch (JAXBUtilException e) {
