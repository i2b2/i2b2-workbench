...
import edu.harvard.i2b2.adminTool.dataModel.PDOItem;
import edu.harvard.i2b2.adminTool.dataModel.PDORequestMessageModel;
import edu.harvard.i2b2.adminTool.dataModel.PDOResponseMessageModel;
import edu.harvard.i2b2.adminTool.dataModel.PDOValueModel;
...
public class IMQueryClient {
	private static final Log log = LogFactory.getLog(IMQueryClient.class);

	private static EndpointReference targetEPR;

	private static String getSetKeyServiceName() {
		return UserInfoBean.getInstance().getCellDataUrl("IM") + "setKey";
	}

	private static String getValidateKeyServiceName() {
		return UserInfoBean.getInstance().getCellDataUrl("IM") + "isKeySet";
	}
	
	private static String getAuditServiceName() {
		return UserInfoBean.getInstance().getCellDataUrl("IM") + "getAudit";
	}
	...
	public static String sendSetKeyQueryRequestREST(String XMLstr) {
		try {
	...
	public static String sendGetAuditQueryRequestREST(String XMLstr) {
		try {
			SAXBuilder parser = new SAXBuilder();
			String xmlContent = XMLstr;
			java.io.StringReader xmlStringReader = new java.io.StringReader(
			...
			MessageUtil.getInstance().setRequest(
					"URL: " + getAuditServiceName() + "\n" + str);//XMLstr);

			OMElement payload = getQueryPayLoad(XMLstr);
			Options options = new Options();

			targetEPR = new EndpointReference(getAuditServiceName());
			options.setTo(targetEPR);
	...
	public static String getPidString(String id) {
		...
		try {
			...
			//	pid = patientRefId;
			//}
			String xmlStr = pdoFactory.requestXmlMessage(id);
			// explorer.lastRequestMessage(xmlStr);
	...
	public static String setKey(String key, String projectID) {
		...
		try {
			...
			//	pid = patientRefId;
			//}
			String xmlStr = pdoFactory.setKeyRequestXmlMessage(key, projectID);
			// explorer.lastRequestMessage(xmlStr);
	...
	public static String isKeySet(String projectID) {
		...
		try {
			...
			//	pid = patientRefId;
			//}
			String xmlStr = pdoFactory.isKeySetRequestXmlMessage(projectID);
			// explorer.lastRequestMessage(xmlStr);
			...
			if (System.getProperty("webServiceMethod").equals("SOAP")) {
				...
				result = IMQueryClient.sendValidateKeyQueryRequestREST(xmlStr);
			}
	...
	public static String getAudit(String userID, String patientID, String projectID, String site) {
		...
		try {
			...
			//	pid = patientRefId;
			//}
			String xmlStr = pdoFactory.getAuditRequestXmlMessage(userID, patientID, projectID, site);
			// explorer.lastRequestMessage(xmlStr);
			...
			if (System.getProperty("webServiceMethod").equals("SOAP")) {
				...
				result = IMQueryClient.sendGetAuditQueryRequestREST(xmlStr);
			}
	...
	public static void main(String[] args) throws Exception {
		...
		String result = sendSetKeyQueryRequestREST(xmlStr);
